export function OperatorPreheater() {
    const container = document.createElement("div");
    container.className = "op-preheater-container hidden h-full grid-cols-12 gap-4 transition-grid overflow-hidden relative";
    container.id = "panel-op-preheater"
    ...
    container.innerHTML = `
                    ...
                    <div class="p-1 flex-1 min-w-0 glass-panel border border-[#2d4a54] bg-[#1a3842] flex flex-row">
                        ...
                        <div class="flex-1 min-w-0">
                            <table class="w-full text-[11px] text-left table-fixed">
                                ...
                                <tbody id="op-table-phpcsec" class="divide-y divide-[#2d4a54]"></tbody>
                            </table>
                        </div>
                    </div>
                    ...
                    <div class="glass-panel flex-1 flex flex-col min-h-0 border border-[#2d4a54] bg-[#1a3842] min-w-0">
                        <div
                            ...
                            <div class="flex gap-2">
                                <button onclick="Actions.clearOpTrendsPreheater()"
                                ...
                                <button onclick="Actions.toggleOpDashChartPreheater('trend')" id="btn-op-pred-trend-preheater"
                                    class="text-[8px] font-bold px-2 py-0.5 bg-[#ebf552] text-whiteoff rounded hover:brightness-110 transition-colors">TREND
                                    VIEW</button>
                                <button onclick="Actions.toggleOpDashChartPreheater('parallel')" id="btn-op-pred-parallel-preheater"
                        ...
                        <div id="op-dash-chart-trend-preheater" class="flex-1 relative w-full min-h-0 p-1"><canvas
                                id="op-summary-chart-canvas-preheater"></canvas></div>
                        <div id="op-dash-chart-parallel-preheater" class="hidden relative w-full flex-1 min-h-0 p-1">
                            <div id="op-dash-plotly-preheater" class="w-full h-full"></div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div
                ...
                <div id="op-live-sensors-list-preheater" class="flex-1 overflow-y-auto p-1 flex flex-col gap-0.5"></div>
            </div>
  `;
