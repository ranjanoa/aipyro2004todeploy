...
export function updateOpSummaryActionsPreheater(data) {
    if (!data) return;
            const insightsBox = document.getElementById('op-summary-insights-preheater');
            ...
            state.opPredictionDataPreheater = {}; // Clear previous predictions
            ...
            if (isFpMode && state.allRecommendations.length > 0 && state.selectedBatchIndex >= 0) {
                ...
                if (rec && rec.fingerprint_prediction) {
                    Object.keys(rec.fingerprint_prediction).forEach(k => {
                        state.opPredictionDataPreheater[k] = rec.fingerprint_prediction[k];
                    });
                }
            }
            ...
            if (data.fingerprint_prediction) {
                Object.keys(data.fingerprint_prediction).forEach(k => {
                    state.opPredictionDataPreheater[k] = data.fingerprint_prediction[k];
                });
            }
            ...
            if (data.actions && data.actions.length > 0) {
                ...
                data.actions.forEach(act => {
                    ...
                    const nspEl = document.getElementById(`op3-pre-nsp-${safeId}`);
                    const tgtEl = document.getElementById(`op3-pre-tgt-${safeId}`);
                    ...
                    if (!state.opPredictionDataPreheater[act.var_name]) {
                        let fakePred = [];
                        for (let m = 0; m <= 15; m++) {
                            if (m <= 5) fakePred.push(liveCurr + (finalTarget - liveCurr) * (m / 5));
                            else fakePred.push(finalTarget);
                        }
                        state.opPredictionDataPreheater[act.var_name] = fakePred;
                    }
                });
            } else {
                window.currentAIVars = [];
                window.latestActions = [];
                document.querySelectorAll('[id^="op3-pre-nsp-"]').forEach(el => el.innerHTML = `<span class="text-white">-</span>`);
                document.querySelectorAll('[id^="op3-pre-tgt-"]').forEach(el => el.innerHTML = `<span class="text-white">---</span>`);
            }
