...
export function updateOpPreheaterLive(data) {
    const list = document.getElementById('op-live-sensors-list-preheater');
    if (!list || !data) return;

    const getTrendBtn = (tag) => {
        const safeId = tag.replace(/[^a-zA-Z0-9]/g, '');
        const isActive = state.opActiveTrendsPreheater.includes(tag);
        return `
            <button onclick="Actions.toggleOpTrendPreheater('${tag}')"
                id="op-trend-btn-preheater-${safeId}"
    ...
    try {
        ...
        const enriched = keys.map(k => {
            ...
            // Reverted: Do not return null if item is missing. 
            // Display all sensors regardless of configuration status.
            let priority = 3; 
            let min = null;
            let max = null;
        ...
        // Sort: Priority 1 (Red) -> Priority 2 (Yellow) -> Priority 3 (Normal)
        enriched.sort((a, b) => {
        ...
        enriched.forEach(item => {
            ...
            const valId = `op-live-val-preheater-${safeId}`;
        ...
        console.error("PreheaterLive update failure suppressed:", e.message);
    }
}